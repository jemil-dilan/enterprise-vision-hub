...
const categories = [
  ...
  { id: "parts", label: "Pièces de Rechange", icon: Wrench },
  { id: "clothing", label: "Vêtements & EPI", icon: Shirt },
  { id: "fuel", label: "Carburant & Lubrifiant", icon: Fuel },
  { id: "medical", label: "Médical", icon: Pill },
  { id: "safety", label: "Sécurité", icon: LifeBuoy },
];
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                <h1 className="text-3xl font-heading font-bold text-foreground">
                  Catalogue Shipchandler
                </h1>
                <p className="text-muted-foreground text-sm mt-1">
                  Navire : <span className="font-semibold text-foreground">{shipInfo.vesselName}</span> • IMO : {shipInfo.imoNumber} • Port : {shipInfo.port}
                </p>
              </div>
        ...
        <div className="container mx-auto px-4 mt-8">
          ...
          {showCart && cart.length > 0 && (
            <motion.div
              ...
              className="bg-card rounded-lg shadow-card p-6 mb-8"
            >
              <h3 className="font-heading font-bold text-foreground mb-4">Récapitulatif du Panier</h3>
              ...
              <div className="flex gap-3 mt-4">
                <Button className="flex-1" onClick={() => navigate("/shipchandler/commande")}>
                  Passer la Commande <ChevronRight size={16} />
                </Button>
                <Button variant="outline" onClick={() => setCart([])}>Vider</Button>
              </div>
            </motion.div>
          )}
          ...
          <div className="relative mb-6">
            ...
            <Input
              placeholder="Rechercher un produit shipchandler..."
              value={search}
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filtered.map((product, i) => (
              <motion.div
                ...
                className="bg-card rounded-lg overflow-hidden shadow-card hover:shadow-card-hover transition-shadow p-5"
              >
                ...
                <h3 className="font-heading font-semibold text-foreground mb-1 text-sm leading-tight">
                  {product.name}
                </h3>
