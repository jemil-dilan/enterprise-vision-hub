    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-heading font-bold text-foreground">{t(r.title, lang)}</h1>
          <p className="text-muted-foreground text-sm mt-1">{t(r.subtitle, lang)}</p>
        </div>

        <form onSubmit={handleRegister} className="bg-card rounded-lg shadow-card p-6 space-y-4">
          <div>
            <label className="text-sm font-medium text-foreground mb-1 block">{t(r.fullName, lang)}</label>
          ...
          <div>
            <label className="text-sm font-medium text-foreground mb-1 block">{t(r.email, lang)}</label>
            <div className="relative">
          ...
          <div>
            <label className="text-sm font-medium text-foreground mb-1 block">{t(r.phone, lang)}</label>
            <div className="relative">
          ...
          <div>
            <label className="text-sm font-medium text-foreground mb-1 block">{t(r.password, lang)}</label>
            <div className="relative">
          ...
          <div>
            <label className="text-sm font-medium text-foreground mb-1 block">{t(r.confirmPassword, lang)}</label>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={16} />
          ...
          <Button type="submit" className="w-full" disabled={loading}>{loading ? t(r.loading, lang) : t(r.submit, lang)}</Button>

          <p className="text-center text-sm text-muted-foreground">
            {t(r.hasAccount, lang)}{" "}
            <Link to="/connexion" className="text-primary hover:underline font-medium">{t(r.login, lang)}</Link>
          </p>
        </form>
      </div>
    </div>
